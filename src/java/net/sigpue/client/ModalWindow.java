...
public class ModalWindow
{
    ...
    private void clearLabel()
    {
        ...
        for (Canvas canv : children)
        {
            if (canv instanceof Label)
            {
                Label label = (Label) canv;
    ...
    private Label createLabel(String message, boolean showLoading)
    {
        ...
        label.setHeight(1);
        label.setStyleName("");
        label.setContents("<img src='images/cargueReporte.gif' heigth=128 width=171><br>"+"<label class='MsnCalifiaR'>"+message+"</label>");
        ...
        if (showLoading)
        {
            ...
            { // show default icon from used skin
                //String icon = Page.getSkinImgDir() + "loading.gif";
                //label.setIcon(icon);
                //label.setIcon("loading-big-transparent.gif");
            }
            if (message.equals(""))
            { // no spacing, just show the loading icon
                // centered
